...
internals.getAvailableProviders = () => {
...
internals.getProviderApi = (server, name, cb) => {
...
internals.handleAuthorised = (req, authData, cb) => {
...
internals.signin = (server, authData, userDoc, cb) => {
...
internals.reconnect = (server, authData, authSession, userDoc, cb) => {
...
internals.saveAuth = (server, authData, userDoc, cb) => {
  ...
  oauth.push(_.extend({ provider }, authData));
  userDoc.oauth = oauth;
  ...
  handler: (req, reply) => {
    ...
    internals.getProviderApi(req.server, name, (err, providerApi) => {
      ...
      providerApi.authenticate(req, (err, authenticateUrl, data) => {
        ...
        reply(err || { authenticateUrl, data });
      });
    });
  }
};
